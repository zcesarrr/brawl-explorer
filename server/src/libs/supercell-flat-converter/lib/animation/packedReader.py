    ...
    def denormalize_transforms(self,
        ...
        for frame_index in range(frame_count):
            for i in range(TranslationChannels):
                value = float(self.to_scalar(bTranslation[i]))
                if flags.has_translation:
                    transform = float(self.to_scalar(
            ...
            for i in range(RotationChannels):
                value = float(self.to_scalar(bRotation[i]))
                if flags.has_rotation:
                    value = float(self.to_scalar(nRotation[i][frame_index])) / 32767.0

                rotation[i][frame_index] = value

            for i in range(ScaleChannels):
                value = float(self.to_scalar(bScale[i]))
